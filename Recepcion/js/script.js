...
const FIRMA_PREDETERMINADA = "img/firma-responsable.png";
...
const FORM_STORAGE_KEY = "taller-mecanico-draft-v1";
const FORM_FIELDS = [
    "cliente",
    "telefono",
    "marca",
...
function restaurarBorrador() {
    try {
        ...
        if (typeof data.vin !== "string" && typeof data.placa_trasera === "string") {
            data.vin = data.placa_trasera;
        }
...
function guardarBorrador() {
    ...
    try {
        ...
        // Si falla el almacenamiento, no bloquea el flujo.
    }
}

function construirJsonRecepcion() {
    const data = {};
    FORM_FIELDS.forEach((id) => {
        const campo = document.getElementById(id);
        if (!campo) return;
    ...
    data.imagenesBase64 = Array.isArray(imagenesBase64) ? imagenesBase64 : [];
    data.version = 1;
    return data;
}
...
function descargarJson(nombreBase, data) {
    ...
    a.href = url;
    a.download = `${limpiarTexto(nombreBase || "reporte")}.json`;
...
function cargarJsonRecepcion(data) {
    if (!data || typeof data !== "object") return;

    FORM_FIELDS.forEach((id) => {
        const campo = document.getElementById(id);
        if (!campo) return;
        if (typeof data[id] === "string") campo.value = data[id];
    });

    if (Array.isArray(data.imagenesBase64)) {
        imagenesBase64 = data.imagenesBase64;
    }
...
function programarGuardado() {
    ...
    draftSaveTimer = setTimeout(guardarBorrador, 400);
}
...
function limpiarBorrador() {
    const confirmado = window.confirm("Seguro que quieres borrar el borrador? Se perderan los datos actuales.");
...
function comprimirImagenDataUrl(dataUrl, { maxWidth = 1600, maxHeight = 1200, quality = 0.82 } = {}) {
...
async function archivoABase64(file) {
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (e) => {
            try {
                const compressed = await comprimirImagenDataUrl(e.target.result, { maxWidth: 1600, maxHeight: 1200, quality: 0.82 });
                resolve(compressed);
            } catch {
...
function buildReportNameRecepcion() {
    const modeloTexto = limpiarTexto((modelo.value || "Modelo").trim() || "Modelo");
    const placaTexto = limpiarTexto((placa_delantera.value || "Placa").trim() || "Placa");
    const clienteTexto = limpiarTexto(formatClientName((cliente.value || "Cliente").trim() || "Cliente"));
    const fecha = formatoFechaArchivo();
    return `RCP_${modeloTexto}_${placaTexto}_${clienteTexto}_${fecha}`;
}
...
async function inicializarRecursosVisuales() {
    // Do not display company name in preview header (logo only)
    const tn = document.getElementById("tallerNombre"); if (tn) tn.textContent = "";
...
function generarPreview() {
    ...
    p_vin.textContent = textoSeguro(vin.value);
    ...
    // Compact preview sections when empty
    const obsSection = document.getElementById("section_observaciones");
    if (obsSection) {
        const isEmptyObs = !observaciones.value || observaciones.value.trim() === "";
        if (isEmptyObs) obsSection.classList.add("empty"); else obsSection.classList.remove("empty");
    }
    ...
    if (!imagenesBase64.length) {
        ...
        const evidenciaSection = document.getElementById("section_evidencia");
        if (evidenciaSection) evidenciaSection.classList.add("empty");
    } else {
        const evidenciaSection = document.getElementById("section_evidencia");
        if (evidenciaSection) evidenciaSection.classList.remove("empty");
    }
...
function softBreakLongWords(text, maxLen = 40) {
    if (!text) return "";
...
function addHeader(pdf, fechaHoraTexto, folioTexto, data) {
    ...
    // Intentionally omit company name and subtitle in PDF header; keep logo centered
    ...
    pdf.line(margin, headerTop + 21.6, pageW - margin, headerTop + 21.6);
}

function addInfoBlock(pdf, y, data) {
    const pageW = pdf.internal.pageSize.getWidth();
    const x = PDF_MARGIN;
    const w = pageW - (PDF_MARGIN * 2);
    const cardY = y + 10;
    const cardH = 68;
    const paddingX = 8;
    const paddingTop = 12;
    ...
    const filas = [
        ...
        ["Año", data.anio, "Placa", data.placaDelantera],
        ["VIN", data.vin, "Kilometraje", data.kilometraje],
...
function addObservaciones(pdf, y, texto) {
    ...
    const w = pageW - (PDF_MARGIN * 2);
    const cardY = y + 10;
    texto = softBreakLongWords(texto || "", 40);
    const lineas = pdf.splitTextToSize(texto, w - 10);
    const lineHeight = 4.4;
    let cardH = Math.max(16, (lineas.length * lineHeight) + 12);
    if (lineas.length === 1 && String(lineas[0] || "").includes("Sin observaciones")) {
        cardH = 18;
    }
    ...
    pdf.setFont(PDF_FONT_NAMES.body, "normal");
    pdf.setFontSize(8.6);
    const mostrar = lineas.length ? lineas : ["Sin observaciones registradas."];
    pdf.text(mostrar, x + 6, cardY + 9.5);
}
...
function addPageNumber(pdf, numeroPagina) {
    ...
    pdf.text(`Pagina ${numeroPagina}`, pageW - PDF_MARGIN, pageH - 5, { align: "right" });
}
...
async function dibujarEvidenciaUnaPagina(pdf, imagenes, yInicio, yFin) {
    ...
    drawSectionHeaderBar(pdf, x, yInicio, w, "Evidencia fotografica");

    if (!imagenes.length) {
        pdf.setFont(PDF_FONT_NAMES.body, "normal");
        pdf.setFontSize(9);
        pdf.setTextColor(...PDF_COLORS.textBody);
        drawGridBox(pdf, x, areaY, w, areaH, 1, 1);
        pdf.text("No se agregaron imagenes de evidencia.", areaX + 2, areaY + 9);
        return;
    }
...
function addObservacionesPaginadas(pdf, yInicial, texto, paginaInicial) {
    ...
    const lineHeight = 4.4;
    texto = softBreakLongWords(texto || "", 40);
    const lineas = pdf.splitTextToSize(texto, w - 6);
    let indice = 0;
    let y = yInicial;
    let pagina = paginaInicial;

    // If there are no real observations, render a minimal block
    if (lineas.length === 0 || (lineas.length === 1 && String(lineas[0] || "").includes("Sin observaciones"))) {
        drawSectionHeaderBar(pdf, x, yInicial, w, "Observaciones de recepcion");
        const cardY = yInicial + 10;
        const cardH = 18;
        drawSectionCard(pdf, x, cardY, w, cardH);
        pdf.setTextColor(...PDF_COLORS.textBody);
        pdf.setFont(PDF_FONT_NAMES.body, "normal");
        pdf.setFontSize(8.6);
        pdf.text(lineas.length ? lineas : ["Sin observaciones registradas."], x + 6, cardY + 9.5);
        return { paginaActual: paginaInicial, siguienteY: cardY + cardH + 6 };
    }
    ...
    while (indice < lineas.length) {
        ...
        const cardY = y + 10;
        const bottom = pageH - PDF_MARGIN - 6;
        const maxCardH = Math.max(30, bottom - cardY);
        const maxLineas = Math.max(1, Math.floor((maxCardH - 12) / lineHeight));
        const chunk = lineas.slice(indice, indice + maxLineas);
        const cardH = Math.max(16, Math.min(maxCardH, (chunk.length * lineHeight) + 12));
        drawSectionCard(pdf, x, cardY, w, cardH);

        const mostrar = chunk.length ? chunk : ["Sin observaciones registradas."];
...
async function dibujarEvidenciaPaginada(pdf, imagenes, yInicial, paginaInicial) {
    ...
    const areaW = w - 4;
    const espacio = 2;
    const bottomConFirma = pageH - PDF_MARGIN - 38;
    const ratio = 16 / 10;
    ...
    // If there are no images, draw a minimal evidence block and return
    if (!imagenes.length) {
        drawSectionHeaderBar(pdf, x, yTitulo, w, "Evidencia fotografica");
        const areaY = yTitulo + 8;
        const areaH = 18;
        drawSectionCard(pdf, x, areaY, w, areaH);
        pdf.setFont(PDF_FONT_NAMES.body, "normal");
        pdf.setFontSize(9);
        pdf.setTextColor(...PDF_COLORS.textBody);
        pdf.text("No se agregaron imagenes de evidencia.", areaX + 6, areaY + 10);
        return { paginaActual: pagina };
    }

    while (index < imagenes.length) {
        const titulo = (index === 0) ? "Evidencia fotografica" : "Evidencia fotografica (continuacion)";
...
async function generarPDF() {
    generarPreview();
    const nombreBase = buildReportNameRecepcion();
    await generarPDFDesdeCero(`${nombreBase}.pdf`);
}

function guardarReporteEditable() {
    const nombreBase = buildReportNameRecepcion();
    descargarJson(`${nombreBase}_editable`, construirJsonRecepcion());
}

function abrirEditorReporte() {
