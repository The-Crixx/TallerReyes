...
function addInfoBlock(pdf, y, data) {
    ...
    drawLabelValue(1, 2, "Año", data.anio);
...
function addObservacionesPaginadas(pdf, yInicial, texto, paginaInicial) {
    ...
    while (indice < lineas.length || (lineas.length === 0 && indice === 0)) {
        ...
        const gridY = y + 8;
        const bottom = 272;
        const gridH = Math.max(26, bottom - gridY);
        drawGridBox(pdf, x, gridY, w, gridH, 1, 1);
        ...
        return { paginaActual: pagina, siguienteY: gridY + gridH + 6 };
    }
...
async function dibujarEvidenciaPaginada(pdf, imagenes, yInicial, paginaInicial) {
    ...
    while (index < imagenes.length || (imagenes.length === 0 && index === 0)) {
        ...
        for (let i = 0; i < capacidad && index < imagenes.length; i++, index++) {
            const fila = Math.floor(i / cols);
            const col = i % cols;
            const drawX = areaX + col * (celdaW + espacio);
            const drawY = areaY + fila * (celdaH + espacio);
...
async function generarPDF() {
    ...
    await exportarPreviewPaginado(previewElement, `${crearFolio()}.pdf`);
}
