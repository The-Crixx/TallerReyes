const NOMBRE_TALLER = "TALLER REYES";
const TECNICO_PREDETERMINADO = "Carlos Reyes";
const TIENDA_PREDETERMINADA = "Taller Reyes";
...
function addHeader(pdf, fechaHoraTexto, folioTexto, data) {
    ...
    pdf.text("Informe de recepcion", leftInfoX, 16);

    pdf.setFont("courier", "bold");
    pdf.setTextColor(...PDF_COLORS.accent);
    pdf.setFontSize(11);
    pdf.text(NOMBRE_TALLER, margin + 186, 16, { align: "right" });

    pdf.setFont("courier", "normal");
    pdf.setTextColor(...PDF_COLORS.textBody);
    pdf.setFontSize(9.2);
    pdf.text(`SN: ${data.snReporte}`, leftInfoX, 24);
    pdf.text(`Fecha y hora: ${fechaHoraTexto}`, leftInfoX, 29.5);
    pdf.text(`Cliente: ${data.cliente}`, leftInfoX, 35);
    pdf.text(`Tecnico: ${data.tecnico}`, leftInfoX, 40.5);

    pdf.text(`Folio: ${folioTexto}`, margin + 122, 24);
    pdf.text(`Tienda: ${textoEnUnaLinea(pdf, data.tienda, 56)}`, margin + 122, 29.5);
    pdf.text(`Telefono: ${textoEnUnaLinea(pdf, data.telefono, 56)}`, margin + 122, 35);
    pdf.text(`Pais: ${textoEnUnaLinea(pdf, data.pais, 56)}`, margin + 122, 40.5);
