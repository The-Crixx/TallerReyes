...
function guardarBorrador() {
	...
	data.conceptos = filas.map(row => {
		...
		const claveSat = row.querySelector('.clave-sat')?.value || '';
		const desc = row.querySelector('.desc')?.value || '';
		const price = row.querySelector('.price')?.value || '';
		return [qty, unitType, claveSat, desc, price];
	});
...
function crearFila(data = []) {
	...
	tr.innerHTML = `
		...
		<td class="center" data-label="Clave de producto y/o servicio"><input type="text" value="${data[2] ?? ""}" class="clave-sat" placeholder="Ej: 01010101"></td>
		<td data-label="Descripcion"><input type="text" value="${data[3] ?? ""}" class="desc"></td>
		<td data-label="Precio unitario"><input type="number" min="0" step="0.01" value="${data[4] ?? ""}" class="price right"></td>
...
function crearFilaPreview(data = []) {
	...
	tr.innerHTML = `
		...
		<td class="center" data-label="Clave de producto y/o servicio"><span class="preview-value clave-sat-preview">${data[2] ?? ""}</span></td>
		<td data-label="Descripcion"><span class="preview-value desc-preview">${data[3] ?? ""}</span></td>
		<td data-label="Precio unitario"><span class="preview-value price-preview">${money(data[4] ?? 0)}</span></td>
...
function rowElements(row) {
	return {
		...
		claveSat: row.querySelector(".clave-sat"),
...
function refrescarVistaPreviaConceptos() {
	...
	if (!rows.length) {
		const tr = document.createElement("tr");
		tr.innerHTML = '<td colspan="6" class="center" data-label="Conceptos">Sin conceptos capturados</td>';
...
function obtenerDatosPDF() {
	const filas = Array.from(conceptosBody.querySelectorAll("tr")).map((row) => {
		...
		const claveSat = clean(refs.claveSat?.value, "-");
...
function drawTableHeader(pdf, x, y, widths) {
	...
	pdf.setFontSize(8.3);
	pdf.setTextColor(...PDF_COLORS.textTitle);
	const labels = ["Cantidad", "Unidad", "Clave de producto y/o servicio", "Descripcion", "Precio Unitario", "Importe"];
	let cursorX = x;
...
function drawConceptRow(pdf, x, y, widths, row) {
	...
	const descWidth = widths[3] - padding * 2;
	const descLines = pdf.splitTextToSize(row.desc || "-", descWidth);
	const unitLines = pdf.splitTextToSize(unitLabel, widths[1] - padding * 2);
	const claveLines = pdf.splitTextToSize(row.claveSat || "-", widths[2] - padding * 2);
	const lineCount = Math.max(Array.isArray(descLines) ? descLines.length : 1, Array.isArray(unitLines) ? unitLines.length : 1, Array.isArray(claveLines) ? claveLines.length : 1);
	...
	for (let i = 0; i < widths.length; i++) {
		...
		if (i === 3) {
			...
			pdf.text(claveLines, cursorX + widths[i] / 2, y + 4.5, { align: "center" });
		} else if (i === 0) {
			pdf.setFont("helvetica", "bold");
...
async function generarPDF() {
	try {
		...
		const tableWidths = [16, 22, 30, 55, 28, 39];
		...
		for (const row of data.conceptos) {
			const rowHeight = Math.max(8, 5 + Math.max((pdf.splitTextToSize(row.desc || "-", tableWidths[3] - 3) || []).length, 1) * 3.8);
...
const btnGuardarEditable = document.getElementById("btnGuardarEditable");
if (btnGuardarEditable) {
	btnGuardarEditable.addEventListener("click", () => {
		const data = obtenerDatosPDF();
		const nombreBase = buildReportNameCotizacion(data);
